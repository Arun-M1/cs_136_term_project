// Compile
// clang gaussian_plus_canny.c ../netpbm.c -I.. -o gaussian_canny_combo -lm
//
// Run:
// ./gaussian_canny_combo input.pgm output_edges.pgm
...
Matrix createGaussianFilter(void);
Matrix convolve(Matrix m1, Matrix m2);
Image applyGaussian(Image img);
Image canny(Image img);

int main(int argc, char *argv[]) {
    if (argc != 3 && argc != 4) {
        printf("Usage: %s input.pgm output_edges.pgm [output_blurred.pgm]\n", argv[0]);
        return 1;
    }
    ...
    Image blurred = applyGaussian(input);

    // Optional: save the blurred image so you can show the intermediate result.
    if (argc == 4) {
        writeImage(blurred, argv[3]);
    }

    // run canny
    Image edges = canny(blurred);

    writeImage(edges, argv[2]);
...
Image applyGaussian(Image img) {
    ...
    Matrix smoothedMatrix = smoothing_filter(imageMatrix, gaussianFilter);
    Image smoothedImage = matrix2Image(smoothedMatrix, 1, 1.0);
